...
if st.session_state.get("show_new_sup") or st.session_state.get("editing_sup"):
    ...
    with st.form("sup_form", clear_on_submit=True):
        ...
        with r1c2:
            products = st.text_input("可做产品/零部件", value=edit_data["products"] if is_edit else "")
        ...
        with s1:
            supports_engraving = st.checkbox("支持刻字", value=bool(edit_data["supports_engraving"]) if is_edit else False)
        with s2:
            supports_logo = st.checkbox("支持LOGO印刷", value=bool(edit_data["supports_logo"]) if is_edit else False)
        with s3:
            supports_locator_parts = st.checkbox("支持定位器配件", value=bool(edit_data["supports_locator_parts"]) if is_edit else False)
        ...
        if submitted:
            data = {
                ...
                "supports_engraving": supports_engraving, "supports_logo": supports_logo,
                "supports_locator_parts": supports_locator_parts,
        ...
        for s in status_sups:
            with st.container():
                ...
                with sc1:
                    ...
                    # 特殊服务标签
                    services = []
                    if s["supports_engraving"]: services.append("刻字")
                    if s["supports_logo"]: services.append("LOGO")
                    if s["supports_locator_parts"]: services.append("定位器配件")
                    if services:
                        st.markdown(" ".join([f":blue[{t}]" for t in services]))
                with sc2:
                    if s["quote_price"]:
                ...
                with sc3:
                    ...
                    with ec2:
                        if st.button("🗑️", key=f"del_sup_{s['id']}"):
                            st.session_state["delete_sup_confirm"] = s["id"]
                            st.rerun()
                
                if st.session_state.get("delete_sup_confirm") == s["id"]:
                    st.warning(f"确认删除供应商「{s['name']}」？")
                    if st.button("确认删除", type="primary", key=f"confirm_del_sup_{s['id']}"):
                        db.delete_supplier(s["id"])
                        st.session_state.pop("delete_sup_confirm", None)
                        st.success("已删除")
                        st.rerun()
